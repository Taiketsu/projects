...
public class EmployeeMenu extends Menu{
	...
	private void getDetails(User user) {
		System.out.println("Okay please give me an account number you want to use!");
		
		String input = INPUT.nextLine();
		
		while(!isInt(input)) {
			System.out.println("Sorry but that isn't a number please try again.");
		...
		while(accountExists(acctID)) {
			System.out.println("I'm sorry that account number already exists");
		...
		System.out.println("How much money are they depositing today?");
		input = INPUT.nextLine();
		...
		System.out.println("Looks like we can use that number!");
		System.out.println("Hang on creating that account now!");
		
		empDAO.approveUser(user, acctID, bal);
		
		System.out.println("Okay I am sending you back to the main menu now!");
		
		MainMenu();
	}
	
	private boolean accountExists(int acctID) {
		Account acct = accDAO.findAccountNumber(acctID);
		if(acct != null) {
			return true;
		}else {
